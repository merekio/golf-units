...
import { CourseRecord, HoleDefinition, updateCourse } from "@/lib/actions/courses";

function buildInitialHoles(holes?: HoleDefinition[] | null): HoleDefinition[] {
  if (holes && holes.length === 18) return holes;
...
export default function EditCourseForm({ course }: EditCourseFormProps) {
  ...
  const [country, setCountry] = useState(course.country ?? "México");
  ...
  const [slopeRating, setSlopeRating] = useState<string>(
    course.slope_rating != null ? String(course.slope_rating) : ""
  );
  const [courseRating, setCourseRating] = useState<string>(
    course.course_rating != null ? String(course.course_rating) : ""
  );
  const [holes, setHoles] = useState<HoleDefinition[]>(buildInitialHoles(course.holes));
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      window.alert("Campo actualizado correctamente");
      router.push("/courses");
      router.refresh();
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Ocurrió un error al actualizar el campo.";
      setError(message);
    } finally {
      setIsSaving(false);
    }
